...
def terminal_set(P, K, c):
    ...
    corners = []
    sign_combinations = np.array(np.meshgrid(*[[-1, 1]] * len(axes_length))).T.reshape(
        -1, len(axes_length)
    )
...
for t in tqdm(range(N_sim), desc="Simulating MPC"):
    ...
    u_0, x_1, x_traj, u_seq = mpc(Ad, Bd, current_horizon, x_hist[t, :], x_ref_horizon, u_ref_horizon, Q, R, P)
    u_hist[t, :] = u_0
